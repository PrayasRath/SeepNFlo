# -*- coding: utf-8 -*-
"""
Created on Thu Jun  9 07:37:36 2022
...
def Khosla_model(LX=200,LY=250,HK=1.0,VKA=1.0,dam_l=40.0,
...
#%%
wdir3=r'O:\Khosla\khosla_nl_mod6'
...
L=np.random.uniform(100,2000,2000)
L=L.astype(int)
dam_l=[0.0]*2000
cutoff=[0.0]*2000
uhead=[0.0]*2000
dhead=[0.0]*2000
aniso=np.random.uniform(-5.0,5.0,2000)
B=np.random.uniform(10,200,2000)
B=B.astype(int)
uhead=np.random.uniform(5,599,2000)
for i in range (2000):
    dam_l[i]=np.random.uniform(10.0,0.8*L[i],1)
    cutoff[i]=np.random.uniform(1.0,B[i],1)
    dhead[i]=np.random.uniform(0,uhead[i],1)
dam_l=np.array(dam_l).ravel()
cutoff=np.array(cutoff).ravel()
dhead=np.array(dhead).ravel()
output_eg1=[0.0]*2000
output_eg2=[0.0]*2000
output_head=[None]*2000
output_frf=[None]*2000
output_fff=[None]*2000
output_DELR=[0.]*2000
output_DELC=[0.]*2000
output_LX=[0.]*2000
output_LY=[0.]*2000
for i in range (2000):
    output_eg1[i],output_head[i],output_frf[i],output_fff[i],
    output_DELR[i],output_DELC[i],output_LX[i],output_LY[i],
    output_eg2[i]=Khosla_model(LX=L[i],LY=B[i],HK=1,VKA=1,dam_l=dam_l[i],
                     d=cutoff[i],HANI=10**aniso[i],NLAY=1,uhead=uhead[i],dhead=dhead[i],modelname='model'+str(i),wdir=wdir3)
import pandas as pd
teg=np.array(output_eg2[:i]).T
data={'Maximum gradient':teg[0],'Exit gradient':teg[1],'Length':L[:i],'Depth':B[:i],'aniso':aniso[:i],
      'DamL':dam_l[:i],'CutOff':cutoff[:i],'Uhead':uhead[:i],'Dhead':dhead[:i]}
my_df = pd.DataFrame(data)
my_df.to_excel('Models5.xlsx')